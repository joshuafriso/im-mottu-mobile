...
class PokemonTypePage extends GetView<PokemonTypeController> {
  const PokemonTypePage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(controller.type), backgroundColor: AppColors().colorType(controller.type)),

      body: Obx(() {
        if (controller.isLoading.value) {
          return LoadingDefault(color: AppColors().colorType(controller.type));
        } else {

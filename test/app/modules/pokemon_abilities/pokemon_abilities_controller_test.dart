...
void main() {
  late PokemonAbilitiesController controller;
  ...
  setUp(() {
    ...
    controller = PokemonAbilitiesController(pokemonRepository: mockPokemonRepository);
    Get.routing.args = 'shed-skin';
  ...
  group("PokemonAbilitiesController - init", () {
    ...
    test("should get arguments in onInit", () async {
      ...
      expect(controller.ability, equals('shed-skin'));
    });
    test("should call getPokemonsByAbility in onInit", () async {
      controller.onInit();

      await Future.delayed(Duration(milliseconds: 100));

      verify(mockPokemonRepository.getPokemonsByAbility(controller.ability)).called(1);
    });
  });

  group("getPokemonsByAbility", () {
    test("should load Pokemons successfully", () async {
      when(mockPokemonRepository.getPokemonsByAbility(any)).thenAnswer((_) async => mockPokemonList);
    ...
    test("should handle with error when load Pokemons", () async {
      when(mockPokemonRepository.getPokemonsByAbility(any)).thenThrow(Exception());
